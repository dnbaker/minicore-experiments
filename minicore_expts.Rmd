                      ...
                      sparsity='sparse', logy=F)
{
  ...
  pl2 <- mfilt %>% filter(dataset == 'Cao2m') %>%
    ggplot(aes(x=k, y=value, color=name)) +
      geom_line() + geom_point() + theme_bw() +
      theme(legend.title = element_blank()) +
      labs(y=measure_pretty, title=paste('Cao2m', title_suffix) + scale_shape_manual(values=c(18,20,22)))
  pl3 <- mfilt %>% filter(dataset == 'Cao4m') %>%
    ggplot(aes(x=k, y=value, color=name)) +
      geom_line() + geom_point() + theme_bw() +
      theme(legend.title = element_blank()) +
      labs(y=measure_pretty, title=paste('Cao4m', title_suffix) + scale_shape_manual(values=c(18,20,22)))
  if(logy) {
