...
ps.add_argument("--maxiter", '-M', type=int, default=50)
ps.add_argument("--ncheckins", '-C', type=int, default=3)
...
mbsizes = args.mbsize if args.mbsize else [500, 2500, 5000, -1]
...
def load_ctrs(path):
    ...
    try:
        ...
        try:
            ...
            print("Failed to parse name %s" % path)
            raise
        msr = msr.split(".")[0]
...
def run_opt(mat, ctrs, mbsizes, *, prior, msr, dataset, nthreads):
    ...
    print("nthreads: ", nthreads, file=sys.stderr)
    '''
    for mbsize in mbsizes:
        print("Processing %d in %s" % (mbsize, str(mbsizes)), file=sys.stderr)
        start = gett()
        out = mc.cluster(mat, ctrs, prior=prior, msr=msr, mbsize=mbsize, maxiter=args.maxiter, ncheckins=args.ncheckins)
        stop = gett()
    ...
    from minicore.constants import SENSDICT
    if msr in [0, 1]:
        sens = SENSDICT["VX"]
